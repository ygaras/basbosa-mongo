...
var AutoModels = {
  ...
  createModelClass : function (name, _static, _instance, done) {
      ...
      Basbosa.add(name + 'Model', newClass);
      newClass.initClass(function(err, result) {
          if (err) throw new Error(err);
          done(err, Basbosa(name));
      });
      return newClass;
  },

  getCollection : function(name, done) {
    if (Basbosa.added(name)) return done(Basbosa(name));
    var modelName = name.replace('Collection', '') + 'Model', self = this;
    if (!Basbosa.added(modelName)) {
      this.createModelClass(modelName, {}, {}, function() {
        done(null, self.getCollection(name));
      });
    }
    var collection = Backbone.Collection.extend({
      model : Basbosa(modelName)
    });

    return done(null, Basbosa.add(name, collection));
  }
};
