...
var AutoModels = {
  ...
  watchModelClasses : function() {
    ...
    modelsClasses.forEach(function(modelClass) {
      if (modelClass.__registeredName === 'BasbosaModel' || modelClass.__registeredName === 'BasbosaFsModel') return;
  ...
  watchFsModelClasses : function() {
    var modelsClasses = Basbosa(/Model$/), count = 0, self = this;

    modelsClasses.forEach(function(modelClass) {
      if (modelClass.__registeredName === 'BasbosaFsModel') return;
      if (!modelClass.isFsModel) return;
      self.setCollectionName(modelClass.__registeredName);
      modelClass.initClass();
    });
  },
