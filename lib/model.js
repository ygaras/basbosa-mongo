...
var BackboneMongoStatic = {
  ...
  getDb:function (cb) {
    Basbosa('Logger').trace('Trying to get connection ' + this.connectionName);
  ...
  autoObjectId : function(id) {
    if (typeof id === 'string' && id.length === 24) {
      return new (Basbosa('ObjectId'))(id);
    }
    return id;
  },
  ...
  populateHasMany:function (HasMany, models, cb, res, qOptions) {
    ...
    Basbosa('AutoModels').createCollectionClass(foreignModelName + 'Collection', {}, {}, function() {
      models.each(function (model) {
        modelIds.push(self.autoObjectId(model.id));
  ...
  afterFind : function (err, results, cb) {
...
BackboneMongo = {
  ...
  saveDb : function (cb) {
    ...
    if (this.get('_id')) {
      var ob = this.toJSON();
      delete ob._id;
      if ((this.get('_id').length == 12 || this.get('_id').length == 24)) {
        query = {_id : new (Basbosa('ObjectId'))(self.get('_id'))};
      } else {
        query = {_id : self.get('_id')};
      }

      // TODO: Check why safe must be false for cb to run
      return coll.update(query, {$set : ob}, {upsert : true, safe : true}, cb);
    }
    coll.insert(this.toJSON(), {safe : true}, function (err, result) {
      if (!err && result) {
        result = result.pop();
  ...
  updateDb : function(params, cb) {

    this.constructor.collection.update({_id : (new Basbosa('ObjectId'))(this.id)}, {'$set' : params}, cb || function(err) {
      if (err) B('Logger').warn(err);
    });
  },
  ...
  deleteDb : function(cb) {
    return this.constructor.collection.remove({_id : new (Basbosa('ObjectId'))(this.id)}, cb);
  }
};
